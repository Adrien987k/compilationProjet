...
*)
  ...
  let from_file fname sep =
    ...
    let l, sch =
      let l = ref [] in
      let a = Pervasives.input_line fd |> split |> Array.mapi (fun k s ->
        try
    ...
    let rec tuple () =
      try
        let t = Pervasives.input_line fd |> split |> Array.mapi (fun i s -> try Some (V.value_of_string sch.(i) s) with _ -> None) in
